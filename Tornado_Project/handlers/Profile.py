...
class AvatarHandler(BaseHandler):
    ...
    def post(self):
        ...
        try:
            ...
            img_name = None
        if not img_name:
            return self.write(dict{"error":RET.THIRDERR, "errmsg":"qiniu error"})
        try:
            ret = self.db.execute("update ih_user_profile set up_avatar=%s where up_user_id=%s", img_name, user_id)
        except Exception as e:
            logging.error(e)
            return self.write(dict{"error":RET.DBERR, "errmsg":"upload failed"})
        img_url = image_url_prefix + img_name
        self.write({"error":RET.OK, "errmsg":"OK", "url":img_url})
