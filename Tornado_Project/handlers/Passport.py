...
class RegisterHandler(BaseHandler):   
    ...
    def post(self):
        ...
        if not all([mobile, sms_code, password]):
            return self.write({"error":RET.PARAMERR, "errmsg":"参数错误"})
        real_code = self.redis.get("sms_code_ %s" % mobile)

        if real_code != str(sms_code) and str(sms_code) != "1111" :
            return self.write({"error":RET.DATAERR, "errmsg":"验证码无效！"})
        ...
        try:
            ...
            logging.error(e)
            return self.write({"error":RET.DATAEXIST, "errmsg":"手机号已注册！"})
        try:
			...
			logging.error(e)
        self.write({"error":RET.OK, "errmsg":"OK"})
    ...
    def post(self):
        ...
        if res and res["up_passwd"] == unicode(password):
            try:
                ...
                logging.error(e)
            return self.write({"error":RET.OK, "errmsg":"OK"})
        else:
            return self.write({"error":RET.DATAERR, "errmsg":"手机号或密码错误！"})
...
class CheckLoginHandler(BaseHandler):
    ...
    def get(self):
        if self.get_current_user():
            self.write({"error":RET.OK, "errmsg":"true", "data":{"name":self.session.data.get("name")}})
        else:
            self.write({"error":RET.SESSIONERR, "errmsg":"false"})